...
namespace BloxEditor
{
    ...
    public class BloxEd : ScriptableObject
    {
        ...
        private IEnumerator EventDefsLoader()
        {
            ...
            int count = 0;
            #region 查找到的事件类 实例化方法（根据函数事件属性查找）
		...
		private IEnumerator BlockDefsLoader()
        {
            ...
            for (int k = 0; k < assemblies.Length; k++)
            {
                ...
                for (int l = 0; l < exportedTypes.Length; l++)
                {
                    ...
                    {
                        Debug.Log(exportedTypes[l].Name, " BloxEd BlockDefsLoader BloxBlock", Color.magenta);
                    ...
                    if (exportedTypes[l].IsClass && typeof(BloxBlockDrawer).IsAssignableFrom(exportedTypes[l]) && exportedTypes[l].Name != "BloxBlockDrawer")
                    {
                        ...
                        if (customAttributes.Length != 0)
                        {
                            ...
                            for (int m = 0; m < array.Length; m++)
                            {
                                ...
                                {
                                    Debug.Log(exportedTypes[l].ToString(), " BloxEd BloxBlockDrawer ", Color.magenta);
            ...
            for (int i = 0; i < foundBlockTypes.Count; i++)
            {
                ...
                {
                    Debug.Log(foundBlockTypes[i].ToString(), " BloxEd foundBlockTypes ", Color.magenta);
            ...
            try
            {
                ...
                {
                    //suifeng
                    Debug.Log("BlockDefsLoader " + enumerator.Current.ToString(), " BloxED", Color.magenta);
